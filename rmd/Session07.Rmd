...
Based on the prior and data, we can obtain posterior:

```{r}

mu.star <- solve(solve(sigma0) + t(x.mat)%*%x.mat )%*% (solve(sigma0)%*%mu0 + t(x.mat)%*%y)

sigma.star <- solve(solve(sigma0) + t(x.mat)%*%x.mat )

a.star <- a0 + 0.5*(t(mu0)%*%solve(sigma0)%*%mu0 + t(y)%*%y  - t(mu.star) %*%solve(sigma.star)%*%mu.star)
d.star <- d0 + n/2
